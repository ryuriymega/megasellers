...
Selectivity.Templates = {
    ...
    resultItem: function(options) {
        return (
            '<div class="selectivity-result-item" onclick="getFilterParameters(\''+escape(options.text)+'\');" data-item-id="' + escape(options.id) + '">' +
                escape(options.text) +
 ...
 * my custom code
 */ 
function getFilterParameters(filterName){
		//window.alert(filterName);
		if (window.XMLHttpRequest)
		 ...
		 xmlhttp.onreadystatechange=function(){
			if(xmlhttp.readyState==4 && xmlhttp.status==200){
				  ...
				  //
				  //
				  if(responseStr.search("Error, Filter not found")!=-1){
					  window.alert("Error, Filter not found");
				  }else{
					  //window.alert(responseStr);
					  var separator=responseStr.indexOf("|||");
					  var separator1=responseStr.indexOf("{{{");
					  var separator2=responseStr.indexOf("}}}");
					  var searchStringPart=responseStr.slice(0,separator);
					  var MINUSsearchStringPart=responseStr.slice(separator+3,separator1);
					  var minPricePart=responseStr.slice(separator1+3,separator2);
					  var maxPricePart=responseStr.slice(separator2+3,responseStr.length);					  
					  //window.alert(searchstringPart);
					  setupCoockie("searchString",searchStringPart);
					  //window.alert(MINUSsearchStringPart);
					  setupCoockie("MINUSsearchString",MINUSsearchStringPart);
					  //window.alert(minPricePart);
					  if(minPricePart==0.00){
						  minPricePart="";
					  }
					  setupCoockie("minPrice",minPricePart);
					  //window.alert(maxPricePart);
					  if(maxPricePart==0.00){
						  maxPricePart="";
					  }
					  setupCoockie("maxPrice",maxPricePart);
					  open("index.php","_self");
				  }
			}
		 }
		///execute script for get selected filter
		xmlhttp.open("GET","getSelectedFilter.php?filterName="+filterName,true);
